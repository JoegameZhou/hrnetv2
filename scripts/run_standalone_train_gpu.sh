...
then 
    echo "Usage: 
    bash run_standalone_train_gpu.sh [PRETRAINED_PATH] [COCO_ROOT] [MINDRECORD_DIR](option)
    "
exit 1
fi
...
#if [ $# -eq 3 ]
#then
#    mindrecord_dir=$(get_real_path $3)
#    if [ ! -d $mindrecord_dir ]
#    then
...
--device_id=$DEVICE_ID --pre_trained=$PATH1 --device_target="GPU"  &> train.log &
