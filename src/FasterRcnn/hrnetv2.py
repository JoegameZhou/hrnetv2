...
class HighResolutionNet(nn.Cell):
    ...
    def __init__(self, config, num_classes):
        ...
        # stage1
        self.stage1_NUM_CHANNELS = config.STAGE1_NUM_CHANNELS
        self.STAGE1_BLOCK = config.STAGE1_BLOCK
        self.STAGE1_NUM_BLOCKS = config.STAGE1_NUM_BLOCKS
        num_channels = self.stage1_NUM_CHANNELS[0]
        block = blocks_dict[self.STAGE1_BLOCK]
        num_blocks = self.STAGE1_NUM_BLOCKS[0]
        self.layer1 = self._make_layer(block, 64, num_channels, num_blocks)
        stage1_out_channel = block.expansion * num_channels

        # stage2
        self.STAGE2_NUM_BRANCHES = config.STAGE2_NUM_BRANCHES
        self.stage2_NUM_CHANNELS = config.STAGE2_NUM_CHANNELS
        self.STAGE2_BLOCK = config.STAGE2_BLOCK
        num_channels = self.stage2_NUM_CHANNELS
        block = blocks_dict[self.STAGE2_BLOCK]
        num_channels = [
        ...
        self.stage2, pre_stage_channels = self._make_stage(
            config.STAGE2_NUM_MODULES, config.STAGE2_NUM_BRANCHES,
            config.STAGE2_NUM_BLOCKS, config.STAGE2_NUM_CHANNELS,
            config.STAGE2_BLOCK, config.STAGE2_FUSE_METHOD,
            num_channels)

        # stage3
        self.STAGE3_NUM_BRANCHES = config.STAGE3_NUM_BRANCHES
        num_channels = config.STAGE3_NUM_CHANNELS
        block = blocks_dict[config.STAGE3_BLOCK]
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))]
        ...
        self.stage3, pre_stage_channels = self._make_stage(
            config.STAGE3_NUM_MODULES, config.STAGE3_NUM_BRANCHES,
            config.STAGE3_NUM_BLOCKS, config.STAGE3_NUM_CHANNELS,
            config.STAGE3_BLOCK, config.STAGE3_FUSE_METHOD, num_channels)

        # stage4
        self.STAGE4_NUM_BRANCHES = config.STAGE4_NUM_BRANCHES
        num_channels = config.STAGE4_NUM_CHANNELS
        block = blocks_dict[config.STAGE4_BLOCK]
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))]
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            config.STAGE4_NUM_MODULES, config.STAGE4_NUM_BRANCHES,
            config.STAGE4_NUM_BLOCKS, config.STAGE4_NUM_CHANNELS,
            config.STAGE4_BLOCK, config.STAGE4_FUSE_METHOD,
    ...
    def _make_stage(self, num_modules, num_branches, num_blocks,
                    num_channels, block_type, fuse_method,
...
def get_initialize_model(cfg, num_classes):
